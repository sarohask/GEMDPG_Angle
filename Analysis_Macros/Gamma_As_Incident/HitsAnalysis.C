...
void HitsAnalysis::Loop()
    ...
    double binEdge[81] = {10.0e-6,
   	...
   	TH1D* h_PrimaryEnergy = new TH1D("h_PrimaryEnergy"," Primary Energy of Gammas ", 80, binEdge);
    h_PrimaryEnergy->GetXaxis()->SetTitle("Energy (MeV)");
    h_PrimaryEnergy->GetYaxis()->SetTitle("Number of Events"); 
    
    TH1D* h_GammaSensitivity = new TH1D("h_GammaSensitivity"," Gamma Sensitivity vs Kinetic Energy", 80, binEdge);
    ...
    TH2D* h_EnergyInterNumber = new TH2D("h_EnergyInterNumber"," Primary Energy vs Number of Interactions ", 80, binEdge, 40, 0, 40);
