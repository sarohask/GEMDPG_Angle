...
void HitsAnalysis::Loop()
	{
   	...
   	TH1D* h_PrimaryEnergy = new TH1D("h_PrimaryEnergy"," Primary Energy of Neutrons ", 150, binEdge);
    h_PrimaryEnergy->GetXaxis()->SetTitle("Energy (MeV)");
    h_PrimaryEnergy->GetYaxis()->SetTitle("Number of Events");
   	    
   	TH1D* h_NeutronSensitivity = new TH1D("h_NeutronSensitivity"," Neutron Sensitivity vs Kinetic Energy", 150, binEdge);
   	h_NeutronSensitivity->GetXaxis()->SetTitle("Energy (MeV)");
   	h_NeutronSensitivity->GetYaxis()->SetTitle("Neutron Sensitivity");	
   	
   	TH1I* h_NumberInteraction = new TH1I("h_NumberInteraction"," Number of Interactions of Primary Neutron ", 40, 0, 40);
    h_NumberInteraction->GetXaxis()->SetTitle("Number of Interaction");
    h_NumberInteraction->GetYaxis()->SetTitle("Number of Events");
   	
   	TH1D* h_InteractionLength = new TH1D("h_InteractionLength"," Length Traversed within the Detector by Primary Neutron ", 800, 0, 400);
    ...
    TH2D* h_LengthInterNumber = new TH2D("h_LengthInterNumber"," Number of Interaction vs Length Traversed by Primary Neutron ", 40, 0, 40, 1000, 0, 200);
   	h_LengthInterNumber->GetXaxis()->SetTitle("Number of Interactions");
   	h_LengthInterNumber->GetYaxis()->SetTitle("Traversed Length (mm)");	
    
    TH1I* h_NumberNeutron = new TH1I("h_NumberNeutron"," Number of Secondary Neutrons Produced per Event ", 20, 0, 20);
   	...
   	for (Long64_t jentry=0; jentry<nentries;jentry++) 
      	...
      	if (event != event_id)  
      		{      			
      			...
      			if (neutronSensitive == true)
      				{
      					h_NeutronSensitivity->Fill(primEnergy);
      				}
      	...
      	if (event == event_id)
      		{
      			...
      			if (Charge != 0 && TrackID != 1 && *VolumeName != "FakeBottom" && *VolumeName != "FakeTop")																		
      				{ 
 						...
 						// neutron	-- Seconadry
 						if (Charge == 0 && *ParticleName == "neutron" && PDG == 2112 && TrackID != 1 && *VolumeName != "FakeBottom" && *VolumeName != "FakeTop")
 							{ 
 								nNeutron++;
 								track = nextTrack ;
    ...
    h_NumberO16->Fill(nO16);
    h_InteractionLength->Fill(interLength);
   	h_NeutronSensitivity->Divide(h_PrimaryEnergy);
   	
   	// Write Histo
   	h_NeutronSensitivity->Write();
